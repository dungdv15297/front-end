  ...
  <div class="row">
    <div class="col-xl-4 col-lg-6 col-md-6" v-for="(item, index) in displayData" :key="index">
      ...
      <div class="single-room mb-50">
        ...
        <div class="room-img">
          <a>
            <img :src="item.image" alt="" style="height:260px; background-size:cover" v-if="item.status==1"/>
            <img :src="rentedImg" style="height:260px; background-size:cover" v-if="item.status==2"/>
          </a>
        </div>
        <div class="room-caption">
          <h4><a class="limited-label" style="cursor:pointer;color:red;font-family: 'Noto Serif' !important">{{ item.title }}</a></h4>
          <h4><a style="font-family: 'Noto Serif' !important">{{item.contact}}</a></h4>
          <div class="per-night">
            <span><u>{{$t('trendroom.acreage')}}</u>{{item.acreage}} <span>{{$t('trendroom.m2')}}</span></span>
            <br>
            <span><u>{{$t('trendroom.vnd')}}</u>{{item.price}} <span>{{$t('trendroom.month')}}</span></span>
            <br>
          </div>
        </div>
      </div>
